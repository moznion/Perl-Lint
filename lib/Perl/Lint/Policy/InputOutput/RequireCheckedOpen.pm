package Perl::Lint::Policy::InputOutput::RequireCheckedOpen;
use strict;
use warnings;
...
use constant {
    DESC => 'Return value of "open" ignored',
    EXPL => 'Check the return value of "open" for success',
};
            ...
            if ($token_data eq 'Fatal') {
                ...
                if ($next_token_type == REG_LIST) {
                    for ($i += 3; my $token = $tokens->[$i]; $i++) {
                        my $token_type = $token->{type};
                        if ($token_type == REG_EXP && $token->{data} eq 'open') {
                            return [];
                        }
                    ...
                    for ($i += 2; my $token = $tokens->[$i]; $i++) {
                        ...
                        elsif (($token_type == STRING || $token_type == RAW_STRING) && $token->{data} eq 'open') {
                            return [];
                        }
                        else {
                ...
                elsif (($next_token_type == STRING || $next_token_type == RAW_STRING) && $next_token->{data} eq 'open') {
                    last;
                }
            }
